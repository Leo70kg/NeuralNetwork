...
MnistModel::MnistModel(const BaseConfiguration& config) : BaseModel(config), m_config(config)
{
...
void MnistModel::ComputeGradient(const Vector<float>& x, const OneHotVector& y)
{
    int L = m_config.LayerNumber;
...
size_t MnistModel::Eval(const Vector<float>& data)
{
    int L = m_config.LayerNumber;
    for (int i = 0; i < L; i++)
    {
...
bool MnistModel::Load(const std::string& filePath)
{
