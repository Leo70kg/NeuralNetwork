...
bool BSDEConfiguration::Load(const std::string& filePath)
{
	...
	this->parallelMode = config_file_param[1];

	if (this->parallelMode == "Master-Slave")
	{
		this->type = ModelType_MasterSlave;
	}
	else if (this->parallelMode == "Ring-Allreduce"){
		this->type = ModelType_RingAllReduce;
	}
	else {
		printf("Please enter a correct parallel mode in the config file.[Master-Slave/Ring-Allreduce]");
		exit(0);
	}
...
{
	if (type == ModelType_MasterSlave) {
		return std::unique_ptr<BSDEModel>(new BSDEModelMs(*this));
	}
	else {
		return std::unique_ptr<BSDEModel>(new BSDEModelRing(*this));
	}
}
