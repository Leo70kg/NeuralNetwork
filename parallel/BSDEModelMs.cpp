...
/*
BSDEModelMs::BSDEModelMs(const BSDEConfiguration& config) : BSDEModel(config)
{   
	int m_rank;
...
void BSDEModelMs::ComputeGradient(const Equation& equation, const float y, size_t index)
{
    int numOfSubnet = m_config.numTimeInterval - 1;
    int dim = m_config.dim_input;
    int L = m_config.subnetLayerNumber;
...
float BSDEModelMs::Eval(const Equation& equation, size_t index)
{
...
void BSDEModelMs::ClearGradient()
{
    int numOfSubnet = m_config.numTimeInterval - 1;
    int L = m_config.subnetLayerNumber;
...
bool BSDEModelMs::Save(int epoch)
{
    std::ofstream fout(save_file_path, std::ios::app);
	end = MPI_Wtime();
...
float BSDEModelMs::Loss(float y, float y_pred)
{
